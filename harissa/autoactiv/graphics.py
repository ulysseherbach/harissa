...
def plotHistoGenes(Data, image_path, *lgenes, **models):
    ...
    for idgene, gene in genedict.items():
        ...
        for t, timepoint in enumerate(Timepoints):
            ...
            n, bins, patches = ax.hist(Xt, bins, normed=1, facecolor=gris, edgecolor='black')
            prepFigure(ax)
            ...
            n, bins, patches = ax.hist(Xt**alpha, bins, normed=1, facecolor=gris, edgecolor='black', label="Data")
            prepFigure(ax)
...
def plotHistoHill(file,modeldict,nbins=None):
    ...
    n, bins, patches = ax1.hist(m, bins=nbins, normed=1, label="m")
    ax1.legend()
